...
- Updated at (UTC): `2026-04-16 22:13:29Z`
...
2. Для текущего milestone цель не в rehearsal уровня production DR, а в практической защите семейного архива через ручной export discipline.
...
1. Для текущего milestone достаточно ручного экспорта данных базы после заметных изменений в составе семьи, связях и правах доступа.
2. Минимум:
- свежий экспорт после крупных изменений структуры дерева,
- отдельный экспорт перед заметными миграциями,
- сохранение файла вне `Supabase` интерфейса.
...
## 5. Что проверять после manual export

1. Что export действительно создается, а не только "должен создаваться".
2. Что файл/снимок можно открыть и он не пустой.
3. Что есть понятная дата export.
4. Что зафиксированы оба актуальных media path:
...
1. Зафиксировать, где именно лежит свежий экспорт базы.
2. Зафиксировать, где именно лежат backup storage или как они восстанавливаются.
3. Убедиться, что файл экспорта хранится вне платформы и доступен владельцу.
4. Зафиксировать текущий rollout state:
...
1. Для `Slava edition` важно не "идеальное disaster recovery", а рабочая и понятная минимальная схема.
2. Для текущего milestone эта схема означает: медиа защищены через `Cloudflare`, а база защищается ручным экспортом после важных изменений.
