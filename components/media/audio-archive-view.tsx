...
export function AudioArchiveView({ treeId, slug, shareToken, canEdit, media, onMediaChange }: AudioArchiveViewProps) {
  ...
  const handleTrackClick = useCallback((trackId: string) => {
    setActiveTrackId((current) => (current === trackId ? null : trackId));
  }, []);

  const handleTrackChange = useCallback((id: string | null) => {
    setActiveTrackId(id);
  }, []);
            ...
            return (
              ...
              >
                <button
                  ...
                  onClick={() => handleTrackClick(asset.id)}
