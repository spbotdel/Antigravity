...
interface AuthPopupLoginFormProps {
  nextPath: string;
  onSuccess: () => void;
}
...
function AuthPopupLoginForm({ nextPath, onSuccess }: AuthPopupLoginFormProps) {
  ...
  return (
    <form className={styles.authPopupForm} onSubmit={handleSubmit}>
      <label className={styles.authPopupField}>
        Почта
        <input className={styles.authPopupInput} name="email" type="email" required autoComplete="email" placeholder="you@example.com" />
      </label>
      <label className={styles.authPopupField}>
        Пароль
        <input className={styles.authPopupInput} name="password" type="password" required minLength={8} autoComplete="current-password" placeholder="********" />
      </label>
      {error ? <p className={styles.authPopupFormError}>{error}</p> : null}
      <button className={styles.authPopupButton} type="submit" disabled={loading}>
      ...
      {isLoginOpen ? (
        <div className={styles.authOverlay} onClick={() => setIsLoginOpen(false)} role="presentation">
          ...
          >
            ...
            <AuthPopupLoginForm nextPath="/dashboard" onSuccess={handleLoginSuccess} />
